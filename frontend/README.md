...
```
freshfeast/
  backend/    Spring Boot 3 + MySQL/H2 + Spring Security (JWT) REST API
...
**3. Log in** with a seeded demo account (password `password123` for all). These credentials are no longer shown on the login page itself (removed for a cleaner/production-style UI) — keep this list handy instead:
